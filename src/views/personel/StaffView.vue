<template>
  ...
  <!-- 查询 -->
  <el-card>
    <el-form ref="Refstaff" :inline="true" :model="states" class="demo-form-inline">
      <el-form-item label="员工姓名：" prop="name">
        <el-input v-model="states.name" placeholder="请输入员工姓名" clearable />
      </el-form-item>
      <el-form-item label="联系方式:" prop="mobile">
        <el-input v-model="states.mobile" placeholder="请输入联系方式" clearable />
      </el-form-item>
      <el-form-item label="身份证号：" prop="idCard">
        <el-input v-model="states.idCard" placeholder="请输入身份证号" clearable />
      </el-form-item>
      <el-form-item label="所属部门：" prop="departmentId">
        <el-select v-model="states.departmentId" placeholder="请选择所属部门" clearable>
          <el-option v-for="item in departmentData" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="所属岗位：" prop="roleId">
        <el-select v-model="states.roleId" placeholder="请选择所属岗位" clearable>
  ...
  <el-card class="table">
    ...
    <MayTable :tableData="data.tableData" :tableItem="data.tableItem" :identifier="identifier">
      <template #operate="{ data }">
        ...
        <el-button type="primary" size="small" link>
          <span @click="enable(data, (data.enable ? 0 : 1))">{{ !data.enable ? '启用' : '禁用' }}</span>
    ...
    <!-- 分页 -->
    <Pagination @page="Holdepage" @psize="Holdepsize" :page="states.page" :psize="states.pageSize" :total="total">
    </Pagination>
  </el-card>
</template>
